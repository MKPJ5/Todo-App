...
function ChangeStatusBtn({ taskStatus, onClick }: ChangeStatusBtnProp) {
  return (
    <button
      onClick={() => onClick()}
      className={`flex h-10 w-10 items-center justify-center rounded-full border-2 transition-all duration-200 hover:scale-110 hover:cursor-pointer active:scale-90 ${
        taskStatus
